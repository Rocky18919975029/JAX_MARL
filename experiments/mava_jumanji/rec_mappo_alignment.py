...
# This fork adds only actor-side C->A representation alignment.
...
) -> Tuple[LearnerFn[AlignmentLearnerState], Actor, AlignmentLearnerState]:
...
def run_experiment(_config: DictConfig) -> float:
    ...
    if _config.align.distance not in ("ln_mse", "linear_cka"):
        raise ValueError("Alignment distance must be ln_mse or linear_cka")
    if _config.align.coef <= 0 or _config.align.epsilon <= 0:
        raise ValueError("Alignment coefficient and CKA epsilon must be positive")
    ...
    _config.logger.system_name = f"rec_mappo_{_config.align.distance}"
...
@hydra.main(
    ...
    config_name="rec_mappo_alignment.yaml",
    version_base="1.2",
)
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}Recurrent MAPPO + {cfg.align.distance} completed{Style.RESET_ALL}")
    return eval_performance
