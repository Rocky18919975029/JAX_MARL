...
# This fork uses one recurrent actor/optimizer per RWARE agent.
...
def score_from_latent(actor: IndependentActor, params, latent, action, action_mask):
...
def get_learner_fn(
    env: MarlEnv,
    actor_network: IndependentActor,
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _actor_loss_fn(
                    ...
                    # Averaging over agents would otherwise scale each
                    # independent actor's gradient down by the agent count.
                    total_loss = config.system.num_agents * (
...
def learner_setup(
...
) -> Tuple[LearnerFn[ARecLearnerState], IndependentActor, ARecLearnerState]:
    ...
    update_fns = (jax.vmap(actor_optim.update), critic_optim.update, q_optim.update)
...
def run_experiment(_config: DictConfig) -> float:
    ...
    if _config.env.env_name != "RobotWarehouse":
        raise ValueError("This independent-actor ARec protocol is pinned to RWARE")
    ...
    _config.logger.system_name = "rec_mappo_nps_arec"
...
@hydra.main(
    ...
    config_name="rec_mappo_arec_nps.yaml",
    version_base="1.2",
)
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}Independent-actor recurrent MAPPO + ARec completed{Style.RESET_ALL}")
    return eval_performance
