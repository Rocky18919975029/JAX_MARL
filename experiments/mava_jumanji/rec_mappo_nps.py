...
# This fork adds independent recurrent actors and optional C->A alignment.
...
from independent_recurrent_actor import IndependentActor
...
class Actor(BaseActor):
    """Mava's recurrent actor, exposing its pre-head GRU representation."""
...
class AlignmentLearnerState(NamedTuple):
    params: Params
...
def normalize_latent_samples(latent):
    """Parameter-free feature LayerNorm, matching the SMAX alignment protocol."""
...
def alignment_distance(actor_latent, critic_reference, mode: str, epsilon: float = 1e-8):
    """LN-MSE or per-agent linear CKA over one recurrent PPO minibatch.

    Inputs have axes [time, environment/chunk, agent, latent]. RWARE agents
    remain active throughout the episode; unlike SMAX there is no dead-unit
    mask, so every sample contributes. The critic reference is detached by the
    caller. Each agent has a separate CKA sample pool, as in SMAX.
    """
    if actor_latent.shape != critic_reference.shape or actor_latent.ndim != 4:
        raise ValueError("Expected matching [time, batch, agent, feature] latents")
    actor = normalize_latent_samples(actor_latent)
    critic = normalize_latent_samples(critic_reference)
    if mode == "ln_mse":
        return jnp.square(actor - critic).mean()
    if mode != "linear_cka":
        raise ValueError(f"Unknown alignment distance: {mode}")
    num_samples = actor.shape[0] * actor.shape[1]
    if num_samples < 2:
        return jnp.zeros((), dtype=actor.dtype)
    actor = actor.transpose((2, 0, 1, 3)).reshape((actor.shape[2], num_samples, -1))
    critic = critic.transpose((2, 0, 1, 3)).reshape((critic.shape[2], num_samples, -1))
    actor = actor - actor.mean(axis=1, keepdims=True)
    critic = critic - critic.mean(axis=1, keepdims=True)
    cross = jnp.einsum("nbi,nbj->nij", actor, critic)
    actor_self = jnp.einsum("nbi,nbj->nij", actor, actor)
    critic_self = jnp.einsum("nbi,nbj->nij", critic, critic)
    numerator = jnp.square(cross).sum(axis=(-1, -2))
    denominator = jnp.sqrt(jnp.square(actor_self).sum(axis=(-1, -2)))
    denominator *= jnp.sqrt(jnp.square(critic_self).sum(axis=(-1, -2)))
    return (1.0 - numerator / (denominator + epsilon)).mean()


def get_learner_fn(
    env: MarlEnv,
    apply_fns: Tuple[Any, Any],
    update_fns: Tuple[Any, Any],
    config: DictConfig,
) -> LearnerFn[AlignmentLearnerState]:
    """Get the learner function."""
    actor_apply_fn, critic_apply_fn = apply_fns
    actor_update_fn, critic_update_fn = update_fns

    def _update_step(learner_state: AlignmentLearnerState, _: Any) -> Tuple[AlignmentLearnerState, Tuple]:
        ...
        def _env_step(
            learner_state: AlignmentLearnerState, _: Any
        ) -> Tuple[AlignmentLearnerState, Tuple[RNNPPOTransition, Metrics]]:
            ...
            learner_state = AlignmentLearnerState(
        ...
        # C->A alignment uses the rollout critic as a fixed reference throughout
        # all PPO epochs. The actor latent is recomputed under current params.
        obs_and_done = (traj_batch.obs, traj_batch.done)
        _, _, critic_reference = critic_apply_fn(
            params.critic_params,
            traj_batch.hstates.critic_hidden_state[0],
            obs_and_done,
            return_latent=True,
        )
        critic_reference = jax.lax.stop_gradient(critic_reference)
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                traj_batch, advantages, targets, critic_reference = batch_info

                def _actor_loss_fn(
                    actor_params: FrozenDict,
                    traj_batch: RNNPPOTransition,
                    gae: jax.Array,
                    critic_reference: jax.Array,
                    key: chex.PRNGKey,
                ) -> Tuple:
                    ...
                    align_loss = (
                        ...
                        if config.align.distance == "none"
                        else alignment_distance(
                            latent,
                            critic_reference,
                            config.align.distance,
                            config.align.epsilon,
                        )
                    )
                    # The global objective averages agents. Multiplying by N
                    # restores each independent actor's per-agent PPO/aux scale.
                    total_loss = (
                        actor_loss - config.system.ent_coef * entropy
                        + config.align.coef * align_loss
                    ) * config.system.num_agents
                    return total_loss, (actor_loss, entropy, align_loss)
                ...
                actor_loss, (_, entropy, align_loss) = actor_loss_info
                ...
                loss_info = {
                    ...
                    "entropy": entropy,
                    "alignment_distance": align_loss,
                    "alignment_weighted_loss": config.align.coef * align_loss,
                }

                return (new_params, new_opt_state, entropy_key), loss_info

            params, opt_states, traj_batch, advantages, targets, critic_reference, key = (
                update_state
            )
            key, shuffle_key, entropy_key = jax.random.split(key, 3)

            # Shuffle minibatches
            batch = (traj_batch, advantages, targets, critic_reference)
    ...
    def learner_fn(learner_state: AlignmentLearnerState) -> ExperimentOutput[AlignmentLearnerState]:
        """Learner function.
...
def learner_setup(
...
) -> Tuple[LearnerFn[AlignmentLearnerState], IndependentActor, AlignmentLearnerState]:
    ...
    actor_network = IndependentActor(Actor(
    ...
    # Per-agent clipping and Adam moments, matching the separate actor banks.
    update_fns = (jax.vmap(actor_optim.update), critic_optim.update)
    ...
    init_learner_state = AlignmentLearnerState(
        params=params,
...
def run_experiment(_config: DictConfig) -> float:
    """Runs experiment."""
    if _config.env.env_name != "RobotWarehouse":
        raise ValueError("This aligned MAPPO protocol is pinned to Jumanji RWARE")
    if _config.align.distance not in ("none", "ln_mse", "linear_cka"):
        raise ValueError("Alignment distance must be none, ln_mse, or linear_cka")
    if _config.align.epsilon <= 0 or (
        _config.align.coef != 0 if _config.align.distance == "none"
        else _config.align.coef <= 0
    ):
        raise ValueError("None requires zero coefficient; alignment requires a positive one")
    if _config.logger.checkpointing.load_model:
        raise ValueError("Alignment checkpoint restore is not implemented; refusing partial restore")
    _config.logger.system_name = (
        "rec_mappo_nps" if _config.align.distance == "none"
        else f"rec_mappo_nps_{_config.align.distance}"
    )
...
@hydra.main(
    ...
    config_name="rec_mappo_nps.yaml",
    version_base="1.2",
)
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}Independent-actor recurrent MAPPO + {cfg.align.distance} completed{Style.RESET_ALL}")
    return eval_performance
